"""File Browser Widget for BARE IDE.

Provides a file tree view for project navigation with toolbar, path bar,
bookmarks, and navigation history — mirroring the file browser in the
Fragillidae IDE Suite's PLAIN IDE.
"""
...
class FileBrowserWidget(QWidget):
    """File browser widget with toolbar, path bar, bookmarks, navigation
    ...
    def __init__(self, parent=None, theme: UITheme = None, settings: SettingsManager = None):
    ...
    def _setup_ui(self):
        ...
        # A QFrame (not a bare layout) so the header gets a full-width,
        # theme-colored background bar instead of just a text-sized patch —
        # matching the "Files" header below.
        bm_header = QFrame()
        bm_header_layout = QHBoxLayout(bm_header)
        bm_header_layout.setContentsMargins(10, 0, 0, 0)

        bm_label = QLabel("BOOKMARKS")
        bm_label.setStyleSheet("font-weight: bold; font-size: 16px;")
        bm_header_layout.addWidget(bm_label)
        bm_header_layout.addStretch()
        bm_layout.addWidget(bm_header)
        ...
        tree_header_layout.setContentsMargins(8, 3, 8, 3)
    ...
    def _create_new_file(self, directory: str = None):
        ...
        filename, ok = QInputDialog.getText(
            self, "New File", "Enter filename:", text="untitled.bare"
        )
        if ok and filename:
